...
class HomeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        v.findViewById<EditText>(R.id.b_search)
            .setOnKeyListener(View.OnKeyListener { v, keyCode, event ->
                if (keyCode == KeyEvent.KEYCODE_ENTER && event.action == KeyEvent.ACTION_UP) {
                    if (!v.findViewById<EditText>(R.id.b_search).text.toString().trim().isEmpty()) {
                        Repository.ApiBarber().searchBarber(
                            v.findViewById<EditText>(R.id.b_search).text.toString()
                        ).enqueue(object : Callback<List<ApiBarber>> {
                            override fun onResponse(
                                ...
                                if (response.isSuccessful) {
                                    try{
                                        adapters.changeData(response.body()!!)
                                        Toast.makeText(v.context, "Ok", Toast.LENGTH_LONG).show()
                                    } catch (e : Exception){
                                        Toast.makeText(v.context, "Err : " + e.message, Toast.LENGTH_LONG).show()
                                    }
                                } else {
                                    Toast.makeText(
                                        v.context,
                            ...
                            override fun onFailure(call: Call<List<ApiBarber>>, t: Throwable) {
                                Toast.makeText(v.context, "error : " + t.message, Toast.LENGTH_LONG)
                                    .show()
                            }
                        })
                    }
        ...
        Repository.ApiBarber().findAll().enqueue(object : Callback<List<ApiBarber>> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    var dt_barber = response.body()!!
                    adapters = BarberAdapter(v.context, dt_barber)
                    var recyclerView = v.findViewById<RecyclerView>(R.id.homeFragment)
